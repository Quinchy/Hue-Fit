    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        if (response.ok) {
          ...
          onSizeAdded && onSizeAdded(data.message, "success", "Success");
          resetForm();
          ...
          onSizeAdded &&
            onSizeAdded(
              errorData.error || "Failed to add size.",
              "error",
              "Error"
            );
        }
  ...
  const fetchSizes = async () => {
    try {
      ...
      const response = await fetch(
        "/api/maintenance/sizes/get-sizes?page=1&search="
      );
      if (response.ok) {
        const data = await response.json();
        setSizes(data.sizes || []);
      ...
      <DialogContent className="min-w-[40rem]">
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-2">
            ...
            <InputErrorMessage
              error={formik.errors.nextId}
              touched={formik.touched.nextId}
            />
          </div>

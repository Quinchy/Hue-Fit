...
import { Trash2, Asterisk, Upload } from "lucide-react";
...
export default function ProductVariantCard({
  ...
  const disablePngClothe = productType === "FOOTWEAR";

  const handlePngClotheChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      const imageURL = URL.createObjectURL(file);
      setFieldValue(`variants.${variantIndex}.pngClothe`, { file, url: imageURL });
    }
  };
      ...
      <Card className="flex-1 flex flex-col p-5 gap-5">
        ...
        <div className="flex flex-row gap-3">
          <div className="flex flex-col gap-3">
            <Label className="mt-1 font-bold flex flex-row items-center">
              PNG Clothing for Virtual Fitting
            </Label>
            <div
              className={`border-4 border-dashed border-border p-4 rounded-lg min-w-[320px] min-h-[320px] max-w-[320px] max-h-[320px] ${
                disablePngClothe
            ...
            >
              {disablePngClothe ? (
                <div className="text-center text-primary/50 font-extralight mt-32">
                  Virtual fitting not available for footwear
                </div>
              ) : values.variants?.[variantIndex]?.pngClothe ? (
                <Image

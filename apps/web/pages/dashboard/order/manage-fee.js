...
// Validation schema for only feeType and feeAmount
      ...
      <Card className="p-10 flex flex-col gap-8 mb-20">
        <form onSubmit={formik.handleSubmit} className="flex flex-col gap-8">
          <div className="flex flex-col gap-4">
            <CardTitle className="text-2xl font-semibold">Delivery Fee Parameters</CardTitle>
            <div>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="fixedFee" className="font-bold flex flex-row items-center">
                    Fixed Amount
                  </Label>
                </div>
                <div className="flex items-center gap-2">
                  <RadioGroupItem value="percentage" id="percentageFee" />
                  <Label htmlFor="percentageFee" className="font-bold flex flex-row items-center">
                    Percentage
                  </Label>
                </div>
              </RadioGroup>
            ...
            <div className="flex flex-col gap-4">
              <Label htmlFor="feeAmount" className="font-bold flex flex-row items-center">
                Fee Amount {formik.values.feeType === "percentage" ? "( in % )" : "( in Currency )"} <Asterisk className="w-4" />
              </Label>
              <Input
                id="feeAmount"
                type="number"
                placeholder={formik.values.feeType === "percentage" ? "e.g. 10" : "e.g. 50.00"}
                value={formik.values.feeAmount}
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                className={InputErrorStyle(formik.errors.feeAmount, formik.touched.feeAmount)}
              />
              {formik.touched.feeAmount && formik.errors.feeAmount && (
                <InputErrorMessage error={formik.errors.feeAmount} touched={formik.touched.feeAmount} />
              )}
            </div>
          </div>

          <div className="flex justify-end gap-4">
            <Button type="submit" className="w-1/5" disabled={formik.isSubmitting}>
